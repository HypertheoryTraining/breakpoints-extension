...
export function activate(context: vscode.ExtensionContext) {
	...
	async function generateChangeLog(workspaceRoot: string, breakpointDir: string, breakpointName: string): Promise<void> {
		try {
			...
			// Get Git commit history for this breakpoint if available
			let stepCommits: Map<string, string> = new Map();
			if (isGitRepo) {
				try {
					// Get commit history for this breakpoint branch
					const commitLog = await executeGitCommand(`git log --oneline --grep="${breakpointName}" --grep="Auto-commit" --grep="created breakpoint"`, workspaceRoot);
					const commits = commitLog.split('\n').filter(line => line.trim());
					
					// Try to match commits to steps
					for (const commit of commits) {
						const [hash, ...messageParts] = commit.split(' ');
						const message = messageParts.join(' ');
						
						// Find step that matches this commit
						const stepMatch = stepDirs.find(step => message.includes(step));
						if (stepMatch) {
							stepCommits.set(stepMatch, hash);
						}
					}
				} catch (gitError) {
					console.log('Could not retrieve Git commit history:', gitError);
				}
			}
			...
			for (let i = 0; i < stepDirs.length; i++) {
				...
				if (isGitRepo && stepCommits.has(stepDir)) {
					changeLogContent += await generateStepDiff(workspaceRoot, stepCommits.get(stepDir)!, stepDir, i === 0);
				} else if (isGitRepo && i > 0) {
					// Try to get diff between consecutive steps
					changeLogContent += await generateStepDiffFromFiles(workspaceRoot, stepPath, stepDir);
				}
			}
			...
			if (isGitRepo) {
				...
				changeLogContent += `- **Includes Diffs**: Yes\n`;
			}
			changeLogContent += `\n---\n\n`;
	...
	// Helper function to generate Git diff for a specific step
	async function generateStepDiff(workspaceRoot: string, commitHash: string, stepName: string, isInitial: boolean): Promise<string> {
		if (isInitial) {
		...
		try {
			...
			const diff = await executeGitCommand(`git show --no-merges --format="" ${commitHash}`, workspaceRoot);
			
			if (!diff.trim()) {
				return `### Git Changes\n\n*No changes detected in this commit*\n\n`;
			}
			
			let diffContent = `### Git Changes\n\n`;
			diffContent += `<details>\n<summary>Show/Hide Diff for ${stepName}</summary>\n\n`;
			diffContent += '```diff\n';
			diffContent += diff;
			diffContent += '\n```\n\n';
			diffContent += '</details>\n\n';
			...
			return `### Git Changes\n\n*Could not retrieve diff for this step: ${error}*\n\n`;
		}
	}
	
	// Helper function to generate diff from step files (fallback method)
	async function generateStepDiffFromFiles(workspaceRoot: string, stepPath: string, stepName: string): Promise<string> {
		try {
			// Get files in this step
			const files = await getFilesRecursively(stepPath);
			...
			let diffContent = `### Git Changes\n\n`;
			diffContent += `<details>\n<summary>Show/Hide File Contents for ${stepName}</summary>\n\n`;
			
			// Show content of changed files (up to a reasonable limit)
			let fileCount = 0;
			const maxFiles = 5; // Limit to prevent huge diffs
			
			for (const file of files.slice(0, maxFiles)) {
				const relativePath = path.relative(stepPath, file);
				const fileExtension = path.extname(file);
				
				// Only show diff for text files
				const textExtensions = ['.ts', '.js', '.html', '.css', '.json', '.md', '.txt', '.yml', '.yaml'];
				if (!textExtensions.includes(fileExtension.toLowerCase())) {
					continue;
				}
				
				try {
					const content = fs.readFileSync(file, 'utf8');
					// Limit file size to prevent huge output
					const maxContentLength = 2000;
					const displayContent = content.length > maxContentLength 
						? content.substring(0, maxContentLength) + '\n\n... (content truncated)'
						: content;
					
					diffContent += `#### ${relativePath}\n\n`;
					diffContent += '```' + fileExtension.substring(1) + '\n';
					diffContent += displayContent;
					diffContent += '\n```\n\n';
			...
			if (files.length > maxFiles) {
				diffContent += `*... and ${files.length - maxFiles} more files*\n\n`;
			}
			
			if (fileCount === 0) {
				diffContent += '*No text files to display*\n\n';
			}
			...
			return `### Git Changes\n\n*Could not analyze step files: ${error}*\n\n`;
		}
	}
